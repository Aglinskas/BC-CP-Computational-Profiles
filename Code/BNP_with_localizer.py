...
# -*- coding: utf-8 -*-
"""
Created on Mon Nov 18 14:40:35 2019
...
def get_locs(s):
    ''' Returns feat by voxels matrix for input subject '''
    # Grab first scan
    fn = root+loc_temp[0].format(s) # Filename
    ds = masker.fit_transform(fn)

    # Append the other scanss
    for i in np.arange(1,len(loc_temp)):
        fn = root+loc_temp[i].format(s)
        #ads = fmri_dataset(fn,mask=mfn)
        ads = masker.fit_transform(fn)
        ds = np.vstack((ds,ads))
...
model = mixture.BayesianGaussianMixture(max_iter=100000,
                                      n_components=500,covariance_type='diag',
                                      tol=0.0001,
                                      init_params='kmeans',
...
pickle_filename = code_root + 'model_version_' + t_start.second
BNP.to_pickle(pickle_filename)
gzip(pickle_filename)
print('time elapsed')
